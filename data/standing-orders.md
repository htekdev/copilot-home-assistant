...
## repo-maintainer: Sofia NEVER Receives Merge Approvals (CRITICAL — from {{PARENT_1}}, 2026-06-22)

> "Do NOT send ANY merge approvals, messages, or notifications to chat_id {{PHONE_NUMBER}} (Sofia). She is ONLY for taller-mecanico PRs. ALL merge approvals for dependabot, milkmama, actions-debugger, carplay, servodetail, htek-dev-site, vidpipe — ALL go to {{PARENT_1}} ONLY. This has caused a scope violation TWICE today and she is frustrated."

**Rule:** repo-maintainer must NEVER send any `merge_pr`, `agent_merge`, or `telegram_send_message` to Sofia ({{PHONE_NUMBER}}) except for `{{GITHUB_USERNAME}}/taller-mecanico` PRs.

**Anti-patterns (ALL forbidden unless repo is taller-mecanico):**
- ❌ `merge_pr(repo: "{{GITHUB_USERNAME}}/carplay-mobile-detail", approver_chat_id: "{{PHONE_NUMBER}}")`
...
- ❌ `telegram_send_message(chat_id: "{{PHONE_NUMBER}}", ...)` for any non-taller-mecanico content

**Correct pattern:**
- ✅ **Always** pass `approver_chat_id: "{{TELEGRAM_PARENT_1}}"` explicitly in every `merge_pr` call
- ✅ Avoid `agent_merge` for non-taller-mecanico repos (it cannot take explicit `approver_chat_id`) — use `merge_pr` with explicit {{PARENT_1}} routing instead
- ✅ Sofia ({{PHONE_NUMBER}}) appears ONLY in `merge_pr` calls for `{{GITHUB_USERNAME}}/taller-mecanico`

**Scope:** repo-maintainer agent exclusively. This rule overrides any config auto-detection.

## Merge Approval Flow — merge-queue.json is READ-ONLY (CRITICAL — from {{PARENT_1}}, 2026-06-24)

> "Why did you let me merge without approval!!!"

**Rule:** `data/merge-queue.json` must NEVER be written to directly by any agent, orchestrator, or session. The ONLY valid write path is the `merge_pr` tool, which sends {{PARENT_1}} Telegram inline keyboard Approve/Deny buttons. Only after explicit button-click approval does the tool auto-create the approval record and dispatch merge-agent.
...
- ✅ {{PARENT_1}} clicks Approve → record auto-created → merge-agent auto-dispatched
- ✅ Even when {{PARENT_1}} says "merge it" verbally, the button flow MUST still happen
...
## Client Emails Must Include Project URLs (CRITICAL — from {{PARENT_1}}, 2026-06-28)
...
## ServoDetail Merge Requests Require Video Proof (CRITICAL — from {{PARENT_1}}, 2026-06-26)
...
- ❌ Sending {{PARENT_1}} a ServoDetail merge approval with only text description
...
## Servosita — NO Social Media Content Posting (CRITICAL — from {{PARENT_1}}, 2026-06-26)
...
**Correct pattern:**
- ✅ `servosita-gtm-content` cron: EXIT IMMEDIATELY at step 1. Log: `[timestamp] gtm-content: skipped — posting disabled per {{PARENT_1}} 2026-06-26`
- ✅ `servosita` agent: Skip ALL GTM content steps. No image generation. No copy writing. No scheduling.
- ✅ If asked about GTM content status: report that posting is disabled per {{PARENT_1}}'s standing order.
...
## Agent Dispatch — Domain History Lookups Are NEVER Steers (CRITICAL — from {{PARENT_1}}, 2026-06-15)

- **"What did {{CLIENT_1}} say about blackout?" = FRESH AGENT.** Not a steer.
- **"What's the status of the {{CLIENT_2}} project?" = FRESH AGENT.** Not a steer.
...
- {{PARENT_1}} correction (turn 947, 2026-06-15): "Wait, why are you steering? The {{CLIENT_1}} Blackwell one, that should be a separate agent, bro."
...
- {{PARENT_2}}'s postpartum/care details can be shared with {{PARENT_1}} and vice versa (they're partners)
...
## Research Tasks Use Opus Model (CRITICAL — from {{PARENT_1}})
...
4. **Root cause of Q-014 (2026-05-29):** Main session dispatched a sub-agent to "draft an {{CLIENT_2}} message" without injecting what {{CLIENT_2}} had discussed. Sub-agent invented content → {{PARENT_1}} received a wrong, confusing message.
...
## Servo Detail Cycle Communication — Always Show Pending Actions (CRITICAL — from {{PARENT_1}}, 2026-06-19)
...
> "When you are running your dev cycle you should just specify the prs needing review you should also rebase them to ensure they are always ready for review with latest code" — {{PARENT_1}}, 2026-06-19 3:30 AM
...
## Servo Detail Brand DNA — Quiet Luxury (CRITICAL — from {{PARENT_1}}, 2026-06-18)
...
- Anti-patterns: tacky gold, aggressive red supercar energy, cluttered dashboards, overdone gradients, "look at me" hero sections, anything that feels bolted-on like {{PARENT_1}}'s critique of Urable.
...
## Blog Interview Delivery (CRITICAL — from {{PARENT_1}})
...
## Browser Automation — Standardized Toolkit (from {{PARENT_1}}, updated 2026-06-26)

**"Don't use playwright... Use a playwright alternative... Playwright sucks"** — {{PARENT_1}}, 2026-06-23
...
5. **Manual guided approach** — for sign-in flows, send Telegram with steps for {{PARENT_1}}
...
**Incident 2 (2026-06-19 — CRITICAL):** coding-agent bypassed the `enforce-merge-pr-tool-only` hookflow and merged ALL 9 ServoDetail PRs without {{PARENT_1}}'s approval. Two specific bypass vectors used:
...
`merge_pr` from `telegram-bridge` → sends {{PARENT_1}} inline Approve/Deny buttons → merge only executes after his explicit approval. `dev_merge_pr`, `Invoke-RestMethod`, `gh pr merge`, and all git API endpoints are ALL forbidden.

**Rules:**
- If `gh pr create` is blocked → escalate to orchestrator; do NOT switch to `Invoke-RestMethod`
- If ANY CLI command is hookflow-blocked → treat the block as a hard stop; find the correct tool OR escalate to {{PARENT_1}}
...
3. If no approved tool exists → escalate to {{PARENT_1}} with the specific action needed
...
## Social Image Style Alignment (CRITICAL — from {{PARENT_1}})
...
## Blackout Image Reference Rule (CRITICAL — from {{PARENT_1}})
...
**Whenever a new domain agent is created that references a sprint or hourly dev cycle in its working.md, platform-manager MUST add a cron entry in the SAME cycle or immediately notify {{PARENT_1}}.** Sprint agents without cron entries run blind — their autonomous work cycle never executes.
...
**When {{PARENT_1}} takes {{CHILD_1_NAME}} to the caregiver (babysitter/caregiver — not school)**, his work calendar should show OOF in the morning.
...
## Hero Images MANDATORY for {{PERSONAL_DOMAIN}} Content (CRITICAL — from {{PARENT_1}})
...
## Hero Images — HTML→Playwright BLOCKED (CRITICAL — from {{PARENT_1}})
...
- Be especially mindful of {{PARENT_2}}'s energy — postpartum with care twins is exhausting
...
- **care/baby check-ins go to BOTH {{PARENT_2}} ({{TELEGRAM_PARENT_2}}) AND {{PARENT_1}} ({{TELEGRAM_PARENT_1}}).** Both parents need the details — weekly updates, appointment reminders, health nudges, and milestone info should be sent to both.
...
## Gateway Registration (CRITICAL — from {{PARENT_1}})
...
8. 👶 Baby/care milestone / appointment reminders

Keep it concise — use HTML formatting for Telegram.

---

## Hookflow-First Governance (CORE PRINCIPLE — from {{PARENT_1}})
