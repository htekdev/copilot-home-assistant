    ...
    try {
      ...
      if (pending.mode === "direct_merge") {
        ...
        const msgText =
          ...
          `<b>PR:</b> <a href="${prUrl}">#${pending.prNumber}</a>${descPart}${proofPart}\n\n` +
  ...
  tools: [
    ...
    // ── merge_pr ─────────────────────────────────────────────────────────
    {
      ...
      handler: async (args) => {
        ...
        // ── Pre-check: E2E proof + Vercel preview (extension-layer gate) ────
        // Replaces the require-e2e-results hookflow — validates proof directly
        // in the merge_pr flow and returns self-correction instructions on failure.
        const proof = await validateMergeProof(repo, pr_number, ghToken);
        if (!proof.skip && !proof.ok) {
          return JSON.stringify({
           error: proof.error,
           repo,
           pr_number,
        ...
        // Build proof links to append to approval message (if E2E repo)
        let proofLinks = "";
        if (proof.ok && !proof.skip) {
          const parts = [];
          if (proof.videoUrl) parts.push(`🎬 <a href="${proof.videoUrl}">Proof video</a> (${proof.videoKeyword})`);
          if (proof.vercelUrl) parts.push(`🔗 <a href="${proof.vercelUrl}">Vercel preview</a>`);
          if (parts.length) proofLinks = "\n\n" + parts.join("\n");
        }
          ...
          if (result.status === "merged") {
           ...
           await telegramApi("sendMessage", {
             ...
             text:
               ...
               `<i>${result.pr_title || ""}</i>${proofLinks}`,
             parse_mode: "HTML",
           }).catch(() => {});
          }
        ...
        const msgText =
          ...
          `<b>PR:</b> <a href="${prUrl}">#${pr_number}</a>${descPart}${proofLinks}\n\n` +
          `<b>Approver:</b> ${approverLabel}\n\n` +
          `Approve this merge?`;
    ...
    // each in the merge queue, posts PR comments, dispatches the merge-agent.
    {
      ...
      handler: async (args) => {
        ...
        // ── E2E proof validation for each PR (extension-layer gate) ─────────
        for (const pr of enriched) {
          const proof = await validateMergeProof(pr.repo, pr.pr_number, ghToken);
          if (!proof.skip && !proof.ok) {
            return JSON.stringify({
              error: proof.error,
              repo: pr.repo,
          ...
          // Stash proof data for the approval message
          pr._proof = proof;
        }

        // Build approval message
        const requestId = generateRequestId();
        const listLines = enriched.map((p) => {
          let line = `• <a href="${p.url}">${p.repo}#${p.pr_number}</a> — <i>${p.title}</i>`;
          if (p._proof?.ok) {
            if (p._proof.videoUrl) line += ` 🎬`;
            if (p._proof.vercelUrl) line += ` 🔗`;
          }
          return line;
        }).join("\n");
