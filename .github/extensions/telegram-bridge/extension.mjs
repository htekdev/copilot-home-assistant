...
function queueOrSend(session, options, chatId) {
  ...
  if (trySend(session) !== null) return;
  if (trySend(_sessionRef) !== null) return;
  ...
  tools: [
        ...
        properties: {
          ...
          speak: {
            ...
            description:
              ...
              "ALWAYS use this when sending to Hector ({{TELEGRAM_PARENT_1}}). " +
              "Do NOT use for Paula ({{TELEGRAM_PARENT_2}}) — she doesn't use Tasker TTS.",
          },
        },
        required: ["message"],
      },
    ...
    // ── merge_pr ─────────────────────────────────────────────────────────
    {
      ...
      handler: async (args) => {
        ...
        const keyboard = {
          inline_keyboard: [[
            { text: "✅ Merge Now", callback_data: `merge:approve:${requestId}` },
            { text: "🤖 Agent Merge", callback_data: `merge:agent:${requestId}` },
