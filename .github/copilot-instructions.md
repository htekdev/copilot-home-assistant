...
- `linkedin-brand-safety` (ext) — blocks LinkedIn messages claiming Hector uses Claude/ChatGPT/Cursor/non-{{EMPLOYER}} AI tools (CRITICAL brand safety)
...
- `block-sofia-scope-leak` (YAML) — blocks `telegram_send_message`, `merge_pr`, and `agent_merge` calls targeting Sofia ({{TELEGRAM_CAREGIVER}}) unless repo/content is taller-mecanico. Updated 2026-06-22 to add `agent_merge` coverage after scope violation incident.
...
- **Cross-extension enforcement:** Import from `.{{EMPLOYER_PARENT}}/extensions/shared/agent-identity-client.mjs`:
...
- **Skill reference:** See `agent-identity` skill at `.{{EMPLOYER_PARENT}}/skills/agent-identity/SKILL.md` for full use cases (code review enforcement, merge authorization, data ownership, audit trails, rate limiting)
...
## Family Context
- **Hector** — Dad, SE at {{EMPLOYER}}. Telegram ID: {{TELEGRAM_PARENT_1}}
- **Paula** — Mom, postpartum (twins born April 16, 2026 at 29-30 weeks). Telegram ID: {{TELEGRAM_PARENT_2}}
- **Hector Jr** — Son, age 4
...
- **Brand Protection**: Hector is a {{EMPLOYER}} employee. ALL {{GITHUB_USERNAME}} content must protect Copilot/{{EMPLOYER}}/{{EMPLOYER_PARENT}}. Pre-publish brand check required. See `copilot-brand-safety` skill.
...
- **Sofia ({{TELEGRAM_CAREGIVER}}) is ONLY for `{{GITHUB_USERNAME}}/taller-mecanico` PRs.** Every other repo — carplay, milkmama, servodetail, htek-dev-site, actions-debugger, vidpipe, ai-harness, etc. — routes EXCLUSIVELY to Hector ({{TELEGRAM_PARENT_1}}).
...
- **NEVER use `agent_merge` for non-taller-mecanico repos** — it lacks an `approver_chat_id` parameter and cannot be safely forced to Hector. Use `merge_pr` with explicit `approver_chat_id: "{{TELEGRAM_PARENT_1}}"` instead.
- **NEVER `telegram_send_message` to Sofia ({{TELEGRAM_CAREGIVER}})** for non-taller-mecanico content.
- This caused scope violations TWICE on 2026-06-22, frustrating Sofia. Zero tolerance going forward.

### Sofia PR Routing: Technical PRs → Hector (CRITICAL — from Hector, 2026-07-07)
...
- **ALL technical PRs route to Hector ({{TELEGRAM_PARENT_1}}):** E2E tests, refactors, migrations, CI fixes, architecture changes, dependency updates, code cleanup, performance improvements.
- Sofia is NOT technical. Do not send her PRs she cannot visually review or understand.
- When calling `merge_pr` for taller-mecanico: use `approver_chat_id: "{{TELEGRAM_CAREGIVER}}"` ONLY if the PR implements something Sofia specifically requested. Otherwise use `approver_chat_id: "{{TELEGRAM_PARENT_1}}"`.

### Communication
- **SPEAK: TTS**: Messages to Hector ({{TELEGRAM_PARENT_1}}) ALWAYS use `speak` param. NEVER for Paula. 1-2 sentences, no emojis/markdown. See `telegram-communication` skill.
