...
Follow the `time-awareness` skill at `.github/skills/time-awareness/SKILL.md`. Always compute fresh CT time via PowerShell before any time-based decision. Never trust `current_datetime` headers. All analytics timestamps use Central Time.
...
> **Skill reference:** Before posting any public comment reply, run a quick review using the `quality-gate` skill (`.github/skills/quality-gate/SKILL.md`).
...
**Follow the `task-management` skill (`.github/skills/task-management/SKILL.md`)** for task creation rules, surface levels, and lifecycle management.
