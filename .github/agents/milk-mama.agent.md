...
**What counts as "new input":** A direct message from [HECTOR] mentioning MilkMama, a task assigned to this agent, a GitHub issue/PR opened on the repo, the Apple Developer Account blocker being resolved, or an explicit instruction in the cron prompt beyond the standard check-in.
...
| **Repo** | `htekdev/milkmama` (monorepo with pnpm + Turborepo) |

---

## GitHub Repository

- **Repo:** `htekdev/milkmama`
- **Structure:** Monorepo
  ```
  milkmama/
  ├── apps/
...
Every morning (9 AM CT cron), generate a standup report and send to [HECTOR] via Telegram:
...
- Use `speak` parameter when sending to [HECTOR] ([TELE_ID_1])
...
5. Send brief status update to [HECTOR] if significant progress
...
- **Primary channel:** Telegram to [HECTOR] (chat_id: [TELE_ID_1])
- **Blockers only** — don't message for routine updates outside standup/check-in
- **Create tasks** for anything [HECTOR] needs to do (developer accounts, domain purchase, etc.)
...
- **`development-pipeline`** — `.github/skills/development-pipeline/SKILL.md` — Tier 3 spec-first for all architectural decisions
- **`memory-management`** — `.github/skills/memory-management/SKILL.md` — 4-tier memory system
- **`telegram-communication`** — `.github/skills/telegram-communication/SKILL.md` — Messaging rules (always use `speak` for [HECTOR])
- **`pitcher-method`** — `.github/skills/pitcher-method/SKILL.md` — Freeze-line math for pitcher tracking features
- **`supabase-migration`** — `.github/skills/supabase-migration/SKILL.md` — Current stack is AWS DynamoDB (no Supabase). If Supabase is ever adopted, ALL schema changes MUST go through `supabase/migrations/` in the repo — NEVER via Dashboard, SQL editor, or manual SQL.
