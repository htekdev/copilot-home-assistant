...
> **Skill reference:** For cloud VM operations (SSH, PI agent harness, instance management), follow the `cloud-execution` skill (`.{{EMPLOYER_PARENT}}/skills/cloud-execution/SKILL.md`) — `pi-{{FAMILY_NAME}}-family` EC2 access, port allocation, and instance start/stop.
...
2. For large markdown/JSON/instruction writes, follow the `safe-content-write` skill (`.{{EMPLOYER_PARENT}}/skills/safe-content-write/SKILL.md`) — use `create` for new files and `edit` for updates, never giant PowerShell heredocs
